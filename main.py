...
class Main:
  def __init__(self, parent): # Constructor, The __init__() function is called automatically every time the class is being used to create a new object.
      ...
      self.main_frame= Frame (parent, bg = background_color, padx=50, pady=50)
      ...
      self.bg_image= Image.open("mrgs.png")
      self.bg_image= self.bg_image.resize((200,150), Image.ANTIALIAS )
      self.bg_image= ImageTk.PhotoImage(self.bg_image)
      self.main_frame = Frame (parent, bg= background_color)
      self.main_frame.grid()
      self.image_label = Label (self.main_frame, image=self.bg_image, bg=background_color)
      self.image_label.place(x=20, y=20, relheight=1)
      # The main name label heading widget
      self.caption_label = Label (self.main_frame, text="MATHS TEACHER CODE QUIZ", font=("Tw Cen MT","35","bold"),bg="#ce0018",)
      self.caption_label.grid(row=0, padx=20)
      #The label for username to input the username by the users.
      self.usernamez_label = Label (self.main_frame, text="Please enter your username below: ", font=("Times","16"),bg="#ce0018")
      self.usernamez_label.grid(row=1, padx=10, pady=20)
      #The space given to enter the username
      self.entry_box = Entry (self.main_frame)
      self.entry_box.grid(row=2,padx=20, pady=20)
    
       #The Continue button
      self.continue_button = Button (self.main_frame, text="Continue", font=("Times", "13", "bold"), bg="Red", command=self.name_collection)
      self.continue_button.grid(row=6,  padx=20, pady=20)
    
      #The exit button
      self.quit_button = Button (self.main_frame, text="Exit", font=("Times", "15", "bold"), bg="Red", command=self.firstScreen)
...
class Questions:
  def __init__(self,parent):
      ...
      self.questions_frame = Frame (parent, bg= background_color, padx=100, pady=100)
      self.questions_frame.grid()
 
#questions for the quiz
      self.question_label = Label (self.questions_frame, text= questions_answers[qnum][0], font=("Courier","20","bold","underline"), bg=background_color,)
      self.question_label.grid(row=1, padx=10, pady=10,)
       #Holds the value of the radio buttons
      self.var1=IntVar()
       #radio button 1
      self.rb1 = Radiobutton (self.questions_frame, text=questions_answers[qnum][1],font=("Courier","17",), bg=background_color, cursor= "star",value=1,padx=10,pady=10,
                variable=self.var1, indicator = 0, background = "#ff5050",borderwidth="8px", justify="left", )
      self.rb1.grid(column=0,row=2,sticky=W, padx=10, pady=10,)
       #radio button 2
      self.rb2 = Radiobutton (self.questions_frame, text=questions_answers[qnum][2],font=("Courier","17"), bg=background_color,cursor= "star",value=2,padx=10,pady=10,
                variable=self.var1, indicator = 0, background = "#ff5050",borderwidth="8px",justify="right")
      self.rb2.grid(row=3,sticky=W, padx=10, pady=10, )
  
      #radio button 3
      self.rb3 = Radiobutton (self.questions_frame, text=questions_answers[qnum][3],font=("Courier","17"), bg=background_color,cursor= "star",value=3,padx=10,pady=10,
                variable=self.var1, indicator = 0, background = "#ff5050",borderwidth="8px",justify=RIGHT,)
      self.rb3.grid(row=4,sticky=W, padx=10, pady=10)
       #radio button 4
      self.rb4 = Radiobutton (self.questions_frame, text=questions_answers[qnum][4],font=("Courier","17"), bg=background_color,cursor= "star",value=4,padx=10,pady=10,
                variable=self.var1, indicator = 0, background = "#ff5050",borderwidth="8px",justify="right",)
      self.rb4.grid(row=5,sticky=W, padx=10, pady=10,)
       #Confrim Button
      self.quizery_instance = Button (self.questions_frame, text="Confirm", font=("Helevetica","15","bold"),bg="Gold", command=self.test_progress)
      self.quizery_instance.grid(row=6, padx=5, pady=5)
       #score label
      self.score_label = Label (self.questions_frame,text="SCORE", font=("Courier"),bg=background_color,)
...
# The exit button that will appear on the question page which will lead to a pop screen which will say Well Done and soon wil have a scoreboard.
 #The exit button
      self.quit1_button = Button (self.questions_frame, text="Exit", font=("Times", "15", "bold"), bg="Red", command=self.last_screen)
      self.quit1_button.grid(row=18)
 # The coding for the leadboard from the exit page for the exit button on the question page
  def last_screen(self):
      ...
      print(return_string)
      open_lastscreen=Last()
      self.listLabel.config(text=return_string)
